...
class MaxHeap():
    ...
    def parent(self, i:int):
        ...
        # 부모노드의 인덱스는 현재 노드의 인덱스에서 1을 뺀 뒤
    ...
    # heapify를 수행해주어야 한다.
    # 이때, 아래쪽에서 위로 올라가며 heapify를 수행하게 되므로
    ...
    def insert(self, data:int):
        ...
        # 외부에서 호출하지 않고 클래스 내부에서만 호출한다는 의미를 가진다.
        new_data_idx = len(self.heap)-1
        self._heapify_up(new_data_idx)

    # _heapify_up
    def _heapify_up(self, index:int):
        # 1. 부모노드가 존재한다면, 그 값이 현재 노드보다 큰지 검사
        ...
        if has_parent:
            ...
            if current_data > parent_data:
                # 2. 만약 부모노드의 값이 현재 노드보다 작다면 교환을 해준다.
                # 현재 노드가 부모노드보다 그 값이 크다면
                # 교환이 일어난다.
                parent_idx = self.parent(index)
                self.heap[index], self.heap[parent_idx] = \
                    self.heap[parent_idx], self.heap[index]
                # 3. 재귀적으로 상향조정 -> 현재 노드가 위로 올라갔으니
                # 포커스를 옮긴다.
                
                # 교환 후, 현재 노드였던 데이터가 부모노드의 인덱스로
                # 옮겨갔으므로, 부모노드의 인덱스를 대상으로
                # 재귀적으로 heapify_up을 수행한다.
                self._heapify_up(parent_idx)
        else: # 부모노드가 존재하지 않거나
            # 존재하더라도 그 값이 현재 노드보다 크거나 같다면
            # 아무 동작을 취하지 않는다.
            # -> 기본케이스
            pass
    ...
    def extract_max(self, show=False):
        ...
        if len(self.heap) > 0: # pop을 한 뒤에도
            # 요소의 개수가 0개가 아니라면
            self._heapify_down(0, show)
        # 아까 추출한 최대값을 반환
        return max_value

    def _heapify_down(self, index:int, show:bool=False):
        # 1. 현재 노드의 인덱스index가 가장 큰 값을 가진 인덱스라고 가정한 뒤
        largest_idx = index
        # 자식노드의 값들과 비교하여 가장 큰 값을 가진 노드의 인덱스를
        # 찾아낸다.
        # 1-1. 왼쪽 자식과 비교
        ...
        # 2. 만약 처음에 설정했던 인덱스와 마지막에 변경된 가장큰 인덱스가
        # 서로 다르다면, 세 개(부모, 좌우자식)의 노드가
        # 힙속성을 만족하지 못하는 것이므로
        if largest_idx != index:
            ...
            if show: print(f">{self.heap}<")
            # heapify_down 호출
            self._heapify_down(largest_idx, show)
            # 끌어내려진 노드 인덱스를 전달한다.
...
if __name__ == "__main__":
    ...
    # 최대힙 객체에 랜덤으로 생성된 숫자들을 순차적으로 삽입
